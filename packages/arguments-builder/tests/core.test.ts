import { type ArgumentItem, ArgumentType, ArgumentsBuilder, BoxJsType } from '../dist/cjs';

describe('ArgumentsBuilder', () => {
  const testArgs: ArgumentItem[] = [
    {
      key: 'Switch',
      name: '总功能开关',
      defaultValue: true,
  ...
  describe('buildSurgeArguments', () => {
    it('should generate correct Surge arguments', () => {
      ...
      expect(result.argumentsText).not.toContain('Switch'); // 因为 Switch 被排除了
  ...
  describe('buildLoonArguments', () => {
    it('should generate correct Loon arguments', () => {
      ...
      expect(result).not.toContain('Switch'); // 因为 Switch 被排除了
